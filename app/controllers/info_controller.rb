...
class InfoController < ApplicationController
  ...
  skip_before_filter :check_for_terms_and_conditions, :except => [:health_check]
  ...
  def health_check
    ...
    respond_to do |format|
      format.html { render :text => 'Arrivu LMS ok' }
      format.json { render :json => { :status => 'Arrivu LMS ok', :revision => Canvas.revision } }
    end
  end
end
