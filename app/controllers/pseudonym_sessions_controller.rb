    ...
    if @is_cas && !params[:no_auto]
      if params[:ticket]
        ...
        if st.is_valid?
          ...
          if @pseudonym
            ...
            reset_session
            session[:delegated_message] = t 'errors.no_matching_user', "Arrivu LMS doesn't have an account for user: %{user}", :user => st.response.user
      ...
      if params[:account_authorization_config_id]
        if aac = @domain_root_account.account_authorization_configs.find_by_id(params[:account_authorization_config_id])
          ...
          message = t('errors.login_errors.no_config_for_id', "The Arrivu LMS account has no authentication configuration with that id")
  ...
  def destroy
    ...
    if @domain_root_account.saml_authentication? and session[:saml_unique_id]
      ...
      if aac = @domain_root_account.account_authorization_configs.find_by_id(session[:saml_aac_id])
        ...
        logout_current_user
        flash[:message] = t('errors.logout_errors.no_idp_found', "Arrivu LMS was unable to log you out at your identity provider")
      end
    ...
    if @domain_root_account.account_authorization_configs.any? { |aac| aac.saml_authentication? } && params[:SAMLResponse]
      ...
      if @domain_root_account.account_authorization_configs.count > 1
        ...
        if aac.nil?
          ...
          destroy_session
          if @domain_root_account.auth_discovery_url
            message = t('errors.login_errors.unrecognized_idp', "Arrivu LMS did not recognize your identity provider")
            redirect_to @domain_root_account.auth_discovery_url + "?message=#{URI.escape message}"
          else
      ...
      if response.is_valid?
        ...
        if response.success_status?
          ...
          if @pseudonym
            ...
            session[:delegated_message] = t 'errors.no_matching_user', "Arrivu LMS doesn't have an account for user: %{user}", :user => unique_id
